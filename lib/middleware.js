...
var initialize = function(jconfloc, local){
  ...
  if(!local){
    ...
    var statusSock = statusZMQ.sock;
    var heartbeatSock = heartbeatZMQ.sock;
  }
  ...
  setInterval(function(){
    ...
    promise.all(tasks.tasks)
      .then(function(){
        ...
        statusSock.send(['',JSON.stringify(status)]);
        console.log(date,'Status monitor sent');
      });
    heartbeat.runCheck();  
    utils.heartbeat.endpoint = "tcp://" + hostIP + ":" + hbZMQPort;
    heartbeatSock.send(['', JSON.stringify(utils.heartbeat)]);
