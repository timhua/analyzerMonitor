      ...
      if(instance.type.toLowerCase() === 'point' || 
        ...
        if(instance.datasource){
          instance.datasource.forEach(function(ds){
            self.brokerData.sources[instance.type + "s_" + ds] = instance.type + "_" + ds;
            if(instance.ports){
              instance.ports.forEach(function(port){
                self.tasks.push(dq.druidQuery("timeseries",instance.type + "_" + ds, null, hostIP, port));     
...
var initialize = function(jconfloc, local){
  ...
  if(!local){
    ...
    console.log("publishing on zmq channel:", hostIP+":"+zmqPort);
  }
