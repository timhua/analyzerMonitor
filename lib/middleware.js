...
var initialize = function(jconfloc, local){
  ...
  // assigning ZMQ ports
  statusZMQPort = jconf.analyzers.hosts[hostname].status.port || 'undefined';
  hbZMQPort = jconf.healthMonitors.hosts[hostname].status.port || 'undefined';
  ...
  if(local){
    ...
    promise.all(tasks.tasks)
      .then(function(){
        ...
        // status.lastDruidSeg = db.lastSegTimestamp;
        console.log(status);
        process.exit(1);
      });
  }
