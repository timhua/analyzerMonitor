...
var initialize = function(jconfloc, local){
  ...
  if(!local){
    var statusZMQ = new zmq.connect(hostIP, statusZMQPort);
    var heartbeatZMQ = new zmq.connect(hostIP, hbZMQPort);
  }
  ...
  setInterval(function(){
    ...
    promise.all(tasks.tasks)
      .then(function(){
        ...
        status.recordsPerSec = dq.recordsPerSec;
        status.timeBoundary = dq.timeBoundary;
        status.endpoint = "tcp://" + hostIP + ":" + zmqPort;
        statusZMQ.sock.send(['',JSON.stringify(status)]);
        console.log(date,'Status monitor sent');
      });
    heartbeat.runCheck();  
    utils.heartbeat.endpoint = "tcp://" + hostIP + ":" + zmqPort;
    heartbeatZMQ.sock.send(['', JSON.stringify(utils.heartbeat)]);
