...
var heartbeat = {
  runCheck: function(){
    ...
    for(var nodes in heartbeat.heartbeat){
      var node = heartbeat.heartbeat[nodes];
    ...
    if(nodeStatus) {
      heartbeat.heartbeat.runStatus = 'RUNNING';
    } else {
      heartbeat.heartbeat.runStatus = 'STOPPED';
    }
    heartbeat.heartbeat.nodeStatus = nodeStatusCount[0] + " nodes running, " + nodeStatusCount[1] + 
